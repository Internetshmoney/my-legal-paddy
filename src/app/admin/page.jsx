...
export default async function AdminPage() {
  ...
  const [articles, tutors] = await Promise.all([
    listAppwriteArticles(),
    admin.canManageTutors ? listTutors() : Promise.resolve([]),
  ]);
  return <main className="min-h-screen bg-[#f7f5ef] text-zinc-950">
    <header className="border-b bg-white"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div className="min-w-0"><p className="font-semibold">My Legal Paddy Admin</p><p className="truncate text-xs text-zinc-500">{admin.email} · {admin.canManageTutors ? 'Manager' : 'Article editor'}</p></div><div className="flex items-center gap-3"><Link href="/" className="text-sm font-medium">View site</Link><form action={logoutAdmin}><button className="rounded-full bg-zinc-950 px-4 py-2 text-sm text-white">Sign out</button></form></div></div></header>
