...
export default async function AdminPage() {
  ...
  return <main className="min-h-screen bg-[#f7f5ef] text-zinc-950">
    <header className="border-b bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-5"><div><p className="font-semibold">My Legal Paddy Admin</p><p className="text-xs text-zinc-500">{admin.email} · {admin.canManageTutors ? 'Manager' : 'Article editor'}</p></div><div className="flex items-center gap-3"><Link href="/" className="text-sm font-medium">View site</Link><form action={logoutAdmin}><button className="rounded-full bg-zinc-950 px-4 py-2 text-sm text-white">Sign out</button></form></div></div></header>
