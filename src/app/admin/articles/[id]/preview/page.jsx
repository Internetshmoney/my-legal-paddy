...
export default async function ArticlePreviewPage({ params }) {
  ...
  return <main className="min-h-screen bg-[#f7f5ef] text-zinc-950">
    <header className="sticky top-0 z-10 border-b border-zinc-200 bg-[#f7f5ef]/95 backdrop-blur">
      <div className="mx-auto flex max-w-5xl flex-col items-start justify-between gap-4 px-5 py-4 sm:flex-row sm:items-center sm:px-8">
        <div>
        ...
        <div className="flex w-full flex-wrap gap-2 sm:w-auto sm:justify-end">
          {article.status === 'draft' ? <Link href={`/admin/articles/${id}/edit`} className="rounded-full border border-[#b5a05e] bg-[#faf7eb] px-4 py-2 text-sm font-semibold text-[#695a2f]">Edit this draft</Link> : null}
          <Link href="/admin" className="rounded-full bg-zinc-950 px-4 py-2 text-sm font-semibold text-white">Back to articles</Link>
        </div>
      </div>
    </header>
