...
import { submitTutorApplication } from '@/app/admin/actions';
...
export default function TutorApplicationForm() {
  const [state, action, pending] = useActionState(submitTutorApplication, {});

  if (state?.success) {
    return <div className="flex min-h-[420px] flex-col items-center justify-center rounded-3xl border border-[#C9B974]/30 bg-white/[0.05] p-10 text-center"><CheckCircle2 size={54} className="text-[#C9B974]" /><h3 className="mt-5 text-2xl font-semibold">Application received</h3><p className="mt-3 max-w-md leading-7 text-zinc-300">{state.success}</p></div>;
  }

  return (
    <form action={action} className="rounded-3xl border border-white/10 bg-white/[0.05] p-6 sm:p-8">
      ...
      {state?.error && <p role="alert" className="mt-5 rounded-xl bg-red-950/50 p-3 text-sm text-red-200">{state.error}</p>}
      ...
      <button disabled={pending} type="submit" className="mt-7 w-full rounded-full bg-[#C9B974] px-6 py-3.5 font-semibold text-black transition hover:bg-[#d8ca92] disabled:opacity-60">{pending ? 'Sending application…' : 'Apply to become a tutor'}</button>
    </form>
  );
}
