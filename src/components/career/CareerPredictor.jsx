...
import { selectCareerQuestions } from '@/data/careerQuestionBank';
...
export default function CareerPredictor() {
  ...
  const complete = questions.length === 12 && answers.length === questions.length;
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      let recentIds = [];
      try { recentIds = JSON.parse(window.localStorage.getItem('mlp-career-question-history') || '[]'); } catch {}
      const selected = selectCareerQuestions(12, recentIds);
      setQuestions(selected);
      try {
        window.localStorage.setItem('mlp-career-question-history', JSON.stringify([...selected.map(({ id }) => id), ...recentIds].slice(0, 96)));
      } catch {}
    }, 0);
  ...
  if (!questions.length) {
    return <section className="px-6 py-16 sm:py-24"><div className="mx-auto max-w-3xl rounded-[2rem] border border-black/10 bg-white p-10 text-center dark:border-white/10 dark:bg-zinc-900"><Scale className="mx-auto animate-pulse text-[#8f7d4d]" size={32} /><p className="mt-4 font-semibold">Preparing your case files…</p><p className="mt-2 text-sm text-zinc-500">Selecting 12 scenarios from the question bank.</p></div></section>;
  }
  ...
  return <section className="px-6 py-16 sm:py-24"><div className="mx-auto max-w-3xl">
    ...
    <div className="mt-8 overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-[0_30px_90px_-60px_rgba(0,0,0,.45)] dark:border-white/10 dark:bg-zinc-900">
      <div className="border-b border-black/10 bg-[#f3e8bd]/55 p-6 dark:border-white/10 dark:bg-[#2b2517] sm:p-9"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#75643d] dark:text-[#dfd29a]">The situation</p><p className="mt-4 text-lg leading-8 text-zinc-800 dark:text-zinc-200">{question.scenario}</p></div>
